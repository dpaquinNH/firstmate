...
Nobody reads this chat: the captain and the main firstmate see only what is appended to '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.status', and a captain-facing sentence that is not appended there has not been sent.
...
Firstmate steers you through durable message files in '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.inbox'/NNN.msg '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.inbox'/handled/`.
...
Report only true captain-relevant outcomes or a declared wait by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-brief-ev.RJzRRR/state/ev-sm.status'`
...
If you end a turn while a job you started is still running, append the `paused:` line first and `working:` when you pick back up, and bound the wait by re-verifying the job is alive within minutes rather than parking on it indefinitely.
