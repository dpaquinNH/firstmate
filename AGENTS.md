...
A declared `paused:` event means a bounded wait expected to clear on its own, whether on an external dependency or on a long job the worker launched itself, while `blocked:` means firstmate action is needed.
